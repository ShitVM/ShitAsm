...
#### 설명
이 프로시저는 스트림에 UTF-32 문자열 출력할 때 사용합니다.
...
`data`는 `int` 자료형의 배열을 가리킵니다. 배열에 접근하는 것은 가능하나, `data`의 값을 임의로 조작하지는 마세요.

`length`는 문자열의 길이를 저장합니다. 이 값을 임의로 조작하지 마세요.

`capacity`는 배열의 길이를 저장합니다. `capacity ≥ length`가 항상 성립합니다. 이 값을 임의로 조작하지 마세요.
...
#### 설명
이 함수는 문자열 맨 뒤에 문자를 삽입할 때 사용합니다.
...
#### 원형
```
proc concat(dest, src)
```

#### 설명
이 함수는 문자열 맨 뒤에 다른 문자열을 연결할 때 사용합니다. `dest` 뒤에 `src`를 연결합니다.

`dest`는 반드시 `pointer` 자료형이어야 합니다. `dest`가 가리키는 곳의 자료형은 반드시 `String32` 구조체여야 합니다.

`src`는 반드시 `pointer` 자료형이어야 합니다. `src`가 가리키는 곳의 자료형은 반드시 `String32` 구조체여야 합니다.
